...
public class MatlabServer {
    ...
    public MatlabServer() {
        ...
        try {
            ...
            Naming.rebind(String.format("rmi://%s:%d/%s", MatlabCaller.HOST_ADDRESS, Registry.REGISTRY_PORT, MatlabCaller.SERVICE_NAME), matlabCaller);
            System.out.println(String.format("%sMatlabServer is ready", JAMAL_PREFIX));
            startWaitingForCommand();
            Naming.unbind(String.format("rmi://%s:%d/%s", MatlabCaller.HOST_ADDRESS, Registry.REGISTRY_PORT, MatlabCaller.SERVICE_NAME));
    ...
    private void startRMIRegistry() {
        Registry registry;
        try {
            registry = LocateRegistry.getRegistry(MatlabCaller.HOST_ADDRESS, Registry.REGISTRY_PORT);
            registry.list();
        } catch (RemoteException e) {
            try {
                LocateRegistry.createRegistry(Registry.REGISTRY_PORT);
            } catch (Exception e1) {
                System.out.println(String.format("%sProblem starting RMI registry:", JAMAL_PREFIX));
                e.printStackTrace();
            }
        }
    ...
    private void processFunction() {
        ...
        try {
            output = com.mathworks.jmi.Matlab.mtFevalConsoleOutput(matlabFunction.getFunctionName(), matlabFunction.getInputArgs(), matlabFunction.getNumberOfOutputArguments());
        } catch (Exception e) {
